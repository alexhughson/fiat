// Live end-to-end proof against real provider APIs. Run with `bun run test:live`.
// Suites skip when the relevant key is absent so keyless environments can still
// execute the file.
    ...
    () => {
        test("an openai-shaped conversation with a tool round runs on anthropic, and the response converts back", async () => {
            ...
            const anthropicBody = AnthropicTranslator.toBody(
                OpenAIChatTranslator.fromBody(openaiShapedBody),
            );
            const anthropicResponse = await callAnthropic(anthropicBody);
            const openaiShapedResponse = OpenAIChatTranslator.toResponse(
