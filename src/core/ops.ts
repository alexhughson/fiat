...
export type CoreOp =
  ...
  | { op: "llm.tool_call"; id: string; name: string; arguments: Record<string, unknown> }
  | { op: "llm.tool_result"; id: string; content: string; isError?: boolean }
  | { op: "llm.output"; format: "json_schema"; name: string; schema: JsonSchema }
...
const CORE_NAMESPACES = new Set(["llm", "meta", "response"]);

export function namespaceOf(op: Op): string {
  const dot = op.op.indexOf(".");
  if (dot <= 0) throw new Error(`malformed op name "${op.op}" — expected "<namespace>.<name>"`);
